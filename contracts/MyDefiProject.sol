...
contract MyDeFiProject {
	...
	function supply(address cTokenAddress, uint underlyingAmount) external {
		...
		IERC20(underlying).appove(cTokenAddress, underlyingAmount);
	...
	function getMaxBorrow(address cTokenAddress) external view returns(uint) {
		(uint result, uint liquidity, uint shortfall()) = comptorller.getAccountLiquidity(address(this));
		...
		return liquidity / getUnderlyingPrice;
	}
}